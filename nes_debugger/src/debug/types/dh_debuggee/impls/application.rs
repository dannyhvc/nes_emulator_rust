...
impl Application for Debuggees {
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        let mn = movable_nodes::MovableNodes::new();

        let mn = mn
            .push(MovableNodes::new_node_at(
                iced::Point::new(100.0, 100.0),
                iced::widget::text("1").into(),
            ))
            .push(MovableNodes::new_node_at(
                iced::Point::new(100.0, 110.0),
                iced::widget::text("2").into(),
            ))
            .push(MovableNodes::new_node_at(
                iced::Point::new(100.0, 120.0),
                iced::widget::text("3").into(),
            ))
            .push(MovableNodes::new_node_at(
                iced::Point::new(100.0, 130.0),
                iced::widget::text("4").into(),
            ))
            .push(MovableNodes::new_node_at(
                iced::Point::new(100.0, 140.0),
                iced::widget::text("5").into(),
            ));
        mn.into()
    }
}
...
impl iced::widget::canvas::Program<()> for CustomWidget {
    type State = ();
    ...
    ) -> Vec<<iced_renderer::Renderer as iced_graphics::geometry::Renderer>::Geometry>{
        let mut frame =
    ...
    //     let text = Text::new(&self.content).size(30).position(self.position);
