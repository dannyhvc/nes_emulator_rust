...
use super::dh_bus::bus::BUS;
use super::types::{AddrModeMneumonic, CpuFlags, M6502AddrModes};
use super::{HIGH_BYTE, LOOKUP_TABLE, LOW_BYTE};
...
pub mod m6502_opcodes {
    use crate::components::{
        ...
        HIGH_BYTE, LOOKUP_TABLE, LOW_BYTE, TOP_BIT_THRESH,
    };
    ...
    impl M6502Opcodes for CPU {
        ...
        fn ROL(&mut self, bus: &mut BUS) -> u8 {
            self.temp =
                (self.fetch(bus) << 1 | self.get_flag(CpuFlags::C)).into();
        ...
        fn SBC(&mut self, bus: &mut BUS) -> u8 {
            ...
            self.temp =
                self.a as u16 + value + self.get_flag(CpuFlags::C) as u16;
            self.set_flag(CpuFlags::C, self.temp & HIGH_BYTE != 0x0000);
